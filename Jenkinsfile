        ...
        stage("Publich ECR") {
            steps {
                withEnv (["AWS_ACCESS_KEY_ID=${env.AWS_ACCESS_KEY_ID}", "AWS_SECRETS_ACCESS_KEY=${env.AWS_SECRETS_ACCESS_KEY}", "AWS_DEFAULT_REGION=${env.AWS_DEFAULT_REGION}"]) 
                {
                    ...
                    sh 'docker build -t rajesh .'
                    sh 'docker tag rajesh:latest 877131812078.dkr.ecr.ap-south-1.amazonaws.com/rajesh:"$BUILD_ID"'
                    sh 'docker push 877131812078.dkr.ecr.ap-south-1.amazonaws.com/rajesh:"$BUILD_ID"'
                }
            }
        }
    }
